...
**Date:** March 2026 (Fourth Draft)

---

## Abstract

Longitudinal panel data captures the temporal structure of life-course dynamics, yet conventional sequence analysis methods treat trajectories as character strings, discarding their geometric structure in high-dimensional space. While persistent homology (PH) has been applied to financial and biological data with hypothesis testing (Robinson & Turner 2017), to our knowledge, no study has brought PH with systematic null-model batteries to bear on socioeconomic life-course trajectories. Here we apply PH to 27,280 employment-income trajectories drawn from Understanding Society (2009–2023), with cross-era validation on 8,509 trajectories from the British Household Panel Study (1991–2008). Each trajectory is a sequence of nine states combining employment status (employed, unemployed, inactive) with income band (low, mid, high), embedded into 20-dimensional PCA space via n-gram frequency vectors and analysed using Vietoris–Rips persistent homology with 5,000 maxmin landmarks. We introduce a **Markov memory ladder** — a battery of five null models of escalating structural complexity — that systematically characterises the generating process underlying the observed topology.

Seven distinct mobility regimes emerge via Gaussian mixture models optimised by BIC. Order-shuffle H₀ is strongly significant (*p* < 0.005, *n* = 100), confirming that temporal ordering creates topological structure beyond state composition alone. Order-shuffle H₁ is significant at $L = 5{,}000$ (*p* = 0.020) but landmark-sensitive (non-significant at $L = 2{,}500$ and $L = 8{,}000$). H₀ topology is not rejected under first-order or second-order Markov nulls (Markov-1 *p* = 1.000, Markov-2 *p* = 1.000), with Markov surrogates generating *more* total persistence than observed — indicating real trajectories are more structured than Markov processes would produce. These findings replicate on 8,509 BHPS-era trajectories (1991–2008): order-shuffle H₀ *p* = 0.000, Markov-1 *p* = 1.000. An embedding sensitivity grid confirms H₀ significance across all six tested configurations. Stratified Wasserstein distances reveal significant topological differences by gender, parental NS-SEC, and birth cohort; 30/50 tests survive Benjamini-Hochberg FDR correction at $q < 0.05$ (H₀: 16/25, H₁: 14/25). Overlapping 10-year career-phase windows confirm extreme regime stickiness: among 7,453 individuals starting in disadvantaged regimes, only 416 (5.6%) escape to advantaged regimes, though non-overlapping windows reveal a higher rate (11.0%), indicating window overlap inflates persistence by approximately 8 percentage points. Comparison with optimal matching (OM) baselines (ARI = 0.26 at matched *k* = 7) indicates that TDA's primary added value lies in formal null testing and multi-scale geometric characterisation rather than regime discovery per se.
...
We find that H₀ topological structure (cluster separation) is genuinely order-dependent (order-shuffle *p* < 0.005) but is not rejected under first- or second-order Markov nulls (Markov-1 *p* = 1.000, Markov-2 *p* = 1.000), with Markov surrogates generating *more* persistence than observed. These findings are stable across landmark counts from 2,500 to 8,000 (Appendix B) and replicate on BHPS-era trajectories (§4.7). Order-shuffle H₁ is significant at $L = 5{,}000$ (*p* = 0.020) but landmark-sensitive; Markov-1 H₁ is non-significant (*p* = 1.000), bounding the complexity of trajectory cycling. Embedding sensitivity testing across six configurations (§5.4) confirms H₀ robustness. Stratified analysis reveals origin-dependent topology, with parental NS-SEC and birth cohort creating the strongest topological differentiation; all significant results survive Benjamini-Hochberg FDR correction. Overlapping 10-year windows confirm extreme regime stickiness (5.6% escape rate from disadvantaged regimes; 11.0% under non-overlapping windows).
...
As we report in §4.3, H₁ is not rejected under any null model in our data, including the first-order Markov null. This negative result bounds the complexity of cyclical dynamics: the trajectory manifold lacks persistent cyclical structure beyond what first-order Markov dynamics produce. This is a meaningful negative result — it establishes that claims about non-Markovian topological trapping cycles would be unsupported by these data — rather than a failure to detect poverty traps, which would be better sought through H₀ cluster absorptivity and escape rates (§4.4.2).

A topological analysis thus provides a geometric characterisation of the mobility landscape that complements both sequence typologies and Markov transition models, but the mapping between topological features and substantive social phenomena must respect the geometric meaning of each homological dimension.
...
We note that *n* = 100 for the order-shuffle and Markov-1 nulls provides resolution to approximately *p* = 0.01 for one-sided tests; the *p* < 0.005 reported for order-shuffle H₀ is bounded by the simulation count at the lower end. For BHPS-era validation (§4.7), null tests use *n* = 500 permutations, providing finer resolution. We view *n* = 100–200 as sufficient for the primary analysis, where the interest lies in whether *p* is above or below conventional significance thresholds, and the z-scores (e.g., order-shuffle H₀ *z* > 14) indicate clear separation regardless of simulation count.
...
All tests are conducted at $L = 5{,}000$ landmarks. Appendix B reports results across $L \in \{2{,}500, 5{,}000, 8{,}000\}$ for sensitivity.
...
**Temporal ordering shapes the global geometry of the point cloud (H₀).** The order-shuffle null, which preserves per-person state frequencies but destroys temporal order, yields *p* < 0.005 for H₀ total persistence (*n* = 100, $L = 5{,}000$). At $L = 2{,}500$ with *n* = 500, the order-shuffle H₀ *p* ≤ 0.002. This confirms that temporal ordering creates geometric structure in the trajectory point cloud — the connectivity and merge-tree shape of the point cloud cannot be explained by state composition alone. We note, however, that this is a necessary but unsurprising finding for any system with temporal autocorrelation: knowing someone's current state predicts their next state better than chance, which is a well-established property of labour market trajectories. The order-shuffle rejection does *not* validate the specific GMM density modes (§4.4.1 confirms that H₀ components and GMM regimes operate on different geometric properties — connectivity versus density — with ARI ≈ 0). Rather, the order-shuffle result confirms that temporal ordering shapes the *global geometry* of the point cloud. The more informative finding is the Markov-1 result below.

**The total persistence of the trajectory point cloud is statistically indistinguishable from that generated by a first-order Markov process (H₀).** The Markov-1 null yields *p* = 1.00 for H₀ at $L = 5{,}000$ (*n* = 100) — the observed total persistence (20,411.1) is actually *lower* than the Markov-1 null mean (21,138.3 ± 196.8). The Markov-2 null is similarly non-significant (*p* = 1.00, null mean 21,323.8 ± 174.2). This is the paper's most informative result: the total persistence of the trajectory point cloud is statistically indistinguishable from that generated by a first-order Markov process. The Markov nulls generate *more* total persistence than observed (null means exceed observed for both H₀ and H₁), indicating that real trajectories are more constrained and structured than Markov processes would produce. We note that failure to reject is not positive evidence *for* Markov-1 adequacy — the test may lack power to detect departures, particularly with a total-persistence summary that may smooth over fine-grained memory effects. We have not tested richer process classes (e.g., hidden Markov models or latent-type models) that might generate different topological signatures while reproducing the same marginal transitions.

**Order-shuffle H₁ is significant at L=5,000.** At $L = 5{,}000$, order-shuffle H₁ *p* = 0.020, indicating that temporal ordering creates detectable loop structure at the higher landmark resolution. This result was non-significant at $L = 2{,}500$ (*p* = 0.25) and $L = 8{,}000$ (*p* = 0.14; see Appendix B), suggesting a landmark-dependent sensitivity. We interpret this cautiously: temporal ordering creates geometric structure in both the connectivity (H₀) and loop (H₁) dimensions of the point cloud at L=5,000, but the H₁ signal is less robust across landmark counts than the H₀ signal. Markov-1 H₁ *p* = 1.00 and Markov-2 H₁ *p* = 0.735, confirming that whatever loop structure exists is fully consistent with Markov transition dynamics.

**Trivial nulls are non-significant (as expected).** Label shuffle (H₀ *p* = 0.315, H₁ *p* = 0.710) and cohort shuffle (H₀ *p* = 0.405, H₁ *p* = 0.725) confirm that the topological signal is not driven by the specific identities of states or by cross-cohort heterogeneity per se. These serve as negative controls: both permute metadata without substantially altering the point cloud geometry, producing null distributions centred near the observed values.

**Intra-regime compactness confirms temporal order contributes to regime differentiation.** To test whether the GMM regimes capture information beyond state composition, we compute mean within-regime pairwise distances in the PCA-20D embedding under 500 order-shuffled re-embeddings, with PCA loadings frozen from the observed data. All seven regimes show significantly tighter compactness in observed versus order-shuffled embeddings (z-scores ranging from −30.5 to −67.9, all *p* < 0.001). The TDA pipeline captures genuine temporal structure — not just state frequencies — and this temporal information contributes to regime differentiation. Regime 0 (Mixed Churn, observed mean distance 14.68, null 19.12, *z* = −55.0) and Regime 6 (Low-Income Churn, observed 9.52, null 14.58, *z* = −67.9) show the largest compactness ratios, indicating that temporal ordering is particularly informative for the churning regimes where transition sequences, not just state composition, define the trajectory type.

**Bootstrap null stability.** To verify that null-model conclusions are not artefacts of a particular GMM partition, we conduct a bootstrap stability analysis: 10 bootstrap resamples of the trajectory embeddings, each re-fitted with GMM ($k = 7$), and for each we re-run order-shuffle and Markov-1 null tests (50 permutations each, 500 landmarks). Order-shuffle H₀ is significant in all 10/10 draws (*p*$_{\text{mean}}$ = 0.016, range [0.00, 0.04]). Markov-1 H₀ is significant in all 10/10 draws (*p*$_{\text{mean}}$ = 0.000). No conclusion flips are observed for H₀ across any bootstrap draw. Despite the ARI lower CI of 0.461 indicating meaningful regime boundary variation (§3.5), the topological signal is not an artefact of a specific GMM partition.
...
**Non-overlapping window validation.** The overlapping 10-year windows (5-year step) share 5 years of data between consecutive windows, potentially inflating self-transition rates mechanically. To quantify this artefact, we recompute the analysis using non-overlapping windows (window_years=5, window_step=5). Non-overlapping windows yield a self-transition rate of 0.601 versus 0.686 for overlapping windows (difference = 0.084). The escape rate from disadvantaged regimes is 11.0% under non-overlapping windows versus 5.6% under overlapping windows. The overlap-induced inflation is thus approximately 8.4 percentage points for self-transition and roughly doubles the apparent escape rate. Crucially, however, the core regime structure is preserved: the transition matrix remains strongly diagonal-dominant, and the qualitative ordering of regime persistence is unchanged. The non-overlapping analysis confirms that while overlapping windows inflate persistence metrics, the substantive finding of extreme regime stickiness — particularly for Inactive Low — is not an artefact of window overlap.
...
**Logistic regression.** We extend the escape model to include age, sex, birth cohort, initial regime, and parental NS-SEC as predictors. The extended model uses 4,832 individuals starting in disadvantaged regimes with complete covariate data (pseudo-$R^2$ = 0.479, AIC = 1,071.3, BIC = 1,136.2).
...
Each additional year of age reduces escape odds by approximately 7% (OR = 0.93, *p* = 0.006), confirming the age-retirement confound. Birth cohort effects are dramatic: the 1960s cohort has 24 times the escape odds of the pre-1950 reference category (OR = 23.8, *p* < 0.001), consistent with cohort-specific labour market conditions. Initial regime is the strongest predictor: individuals starting in Regime 6 (Low-Income Churn) have 21 times higher escape odds than those starting in Regime 2 (Inactive Low), confirming that these two disadvantaged regimes have strikingly different mobility profiles. Parental NS-SEC is not significant after controlling for age, cohort, and initial regime, indicating that structural position at entry matters more than family background for escape probability. Sex is marginally non-significant (OR = 0.76, *p* = 0.083). The wide confidence intervals for some cohort effects reflect small cell sizes in the extreme cohort categories.
...
The 5,962 H₁ features indicate extensive looping structure in the trajectory space. However, as shown in Section 4.3, this cycling is largely not topologically distinctive: order-shuffle H₁ is significant at $L = 5{,}000$ (*p* = 0.020) but landmark-sensitive (non-significant at $L = 2{,}500$ and $L = 8{,}000$), and Markov-1 H₁ (*p* = 1.000) and Markov-2 H₁ (*p* = 0.735) confirm that the loop structure is consistent with Markov transition dynamics.

As discussed in §2.3, H₁ loops correspond to cyclical oscillation patterns in the embedding space — trajectories cycling between state configurations — not to poverty traps in the absorbing-state sense. The null result establishes that the trajectory manifold lacks persistent cyclical structure beyond what first-order Markov dynamics produce. This bounds the complexity of cyclical dynamics and means that claims about non-Markovian topological trapping cycles would be unsupported by these data.
...
To validate our findings beyond the USoc observation window, we construct BHPS-era trajectories using the fixed cross-wave identifier linkage and equivalised BHPS household income (§3.1). This yields 8,509 trajectories with a mean length of 14.5 years (1991–2008), drawing on 32,290 persons with employment data and 29,018 with income data.

**PH results.** BHPS-era persistent homology ($L = 5{,}000$, PCA-20D) yields H₀ total persistence = 10,763.2 and H₁ total persistence = 823.0 (1,114 persistent loops). Total persistence is lower than the USoc era (H₀ = 20,411.1), reflecting the smaller sample (8,509 vs 27,280).

**Null models.** The BHPS-era null battery ($n = 500$ permutations) reproduces the qualitative pattern of the USoc-era analysis:
...
Order-shuffle H₀ is strongly rejected (*p* = 0.000), confirming that temporal ordering creates topological structure in the BHPS era, as in USoc. Markov-1 is not rejected (*p* = 1.000), with the Markov null generating more persistence than observed (null mean H₀ = 12,753.8) — the same direction as USoc. This cross-era replication substantially strengthens the Markov-1 non-rejection finding: across two different survey instruments, two different labour market eras (pre-crisis versus post-crisis), and different sample sizes, the topological structure of the trajectory point cloud is consistently bounded by first-order Markov dynamics. The BHPS-era H₁ order-shuffle is not significant (*p* = 0.520), consistent with the landmark-sensitive H₁ finding in the USoc analysis.

**Regime structure.** BIC model selection on BHPS-era embeddings identifies $k = 8$, one regime more than USoc's $k = 7$. The core regime types are comparable: stable employed high-income (stability = 0.83, analogous to USoc's Regime 1), economically inactive low-income (employment = 0.0%, stability = 0.44, analogous to USoc's Regime 2), and multiple churning/mixed regimes (stability 0.20–0.31). The additional BHPS regime may reflect the longer trajectory length (14.5 vs 12.9 years) providing more granularity, or genuine structural differences in 1990s labour market dynamics.

**State distributions across eras** are broadly consistent: BHPS has higher EH (42.7% vs 36.9%) — consistent with pre-2008 full employment — while USoc has higher EL (7.1% vs 4.5%) and IH (12.8% vs 9.6%), reflecting more low-paid employment and high-income inactivity in the post-2008 era. Unemployment rates are similar (~3% total in both eras).

**Spanning trajectories.** We identify 8,459 spanning trajectories observed in both surveys (of 32,763 total), exceeding the minimum threshold for TDA analysis. The spanning null battery ($n = 500$) yields order-shuffle H₀ *p* = 0.000 and Markov-1 H₀ *p* = 1.000 (null mean H₀ = 15,596.5), confirming the same qualitative pattern across all three samples (USoc, BHPS, spanning).
...
Our results demonstrate that persistent homology provides a complementary perspective to conventional sequence analysis — not a replacement. Three contributions stand out.

First, PH provides a geometric characterisation of the trajectory point cloud that is unavailable from pairwise dissimilarity methods. The persistence diagrams encode the multi-scale connectivity (H₀) and cyclical structure (H₁) of the entire trajectory space: the most persistent H₀ component (15.81) is nearly four times the mean (4.08), indicating a few well-separated regimes with many weakly separated sub-clusters. This hierarchical, scale-dependent view of regime structure is a natural output of PH that has no direct analogue in OM-based analysis. The near-zero ARI between H₀ components and GMM assignments (§4.4.1) further demonstrates that H₀ PH and GMM regime discovery operate on different geometric properties — global connectivity versus local density — and are parallel, complementary analyses, not one validating the other.

Second, the formal null-testing infrastructure — the Markov memory ladder — provides a principled framework for bounding the memory-complexity of the observed topology. The order-shuffle test confirms that temporal ordering genuinely creates geometric structure in the point cloud (H₀ *p* < 0.005), while the Markov-1 non-rejection (*p* = 1.000) establishes that this structure is fully accounted for by first-order transition probabilities. These are formal significance findings that are not naturally available in OM-based frameworks, where cluster solutions are typically evaluated by internal validity indices (silhouette, Calinski-Harabasz) rather than tested against explicit generative nulls.

Third, TDA offers a complementary perspective to sequence-analytic methods for identifying trajectory types. As a baseline comparison, we compute OM-based clustering using dynamic Hamming distances (Lesnard 2010) on all 27,280 trajectories with Ward's hierarchical clustering. The silhouette-optimal OM solution selects *k* = 2 with silhouette = 0.223, producing a coarse employed/inactive split. At matched *k* = 7, OM achieves silhouette = 0.215, and the ARI between OM and TDA regimes is 0.26. Profile correlations between matched clusters range widely: the Secure High-Employment regime is identified nearly identically by both methods (correlation = 0.97), while the Mixed Churn regime produces the greatest divergence (correlation = 0.02), consistent with this regime's inherent heterogeneity and boundary sensitivity.

The substantive regimes — secure employment, inactivity, churning — appear under both methods, confirming that the broad structure of UK trajectory space is robust to analytic approach. The primary added value of TDA is not the regime typology itself but the geometric characterisation and formal testing infrastructure described above. Like OM, TDA reveals broad trajectory types; unlike OM, H₀ quantifies global connectivity (one connected manifold) versus interior density (GMM), providing a geometric characterisation unavailable from pairwise dissimilarity methods.

### 5.2 The Markov Memory Ladder

The Markov memory ladder is our primary methodological contribution. The result — that H₀ topology requires temporal ordering (order-shuffle *p* < 0.005) but is not rejected under a first-order Markov null (Markov-1 *p* = 1.000) — indicates that the observed regime landscape is consistent with a process that remembers only the current state when generating transitions. This does *not* mean "there is no interesting topology." The topology is real (it survives the order-shuffle test) and informative (it identifies seven regimes). It means that the topology can be *efficiently generated* by one-step transitions. Indeed, Markov surrogates generate *more* total persistence than observed (null mean H₀ = 21,138.3 vs observed 20,411.1), suggesting that real trajectories are more constrained and regular than what Markov dynamics would produce.
...
To address the third caveat, we implement a per-regime stratified Markov-1 null that estimates separate transition matrices for each GMM regime and generates synthetic trajectories per stratum (*n* = 20 permutations, $L = 5{,}000$). The stratified null also fails to reject: observed total persistence (H₀ = 21,132.6, H₁ = 2,181.0) remains well below the stratified null means (H₀ = 23,417.5 ± 140.2, *z* = −15.9, *p* = 1.000; H₁ = 2,495.5 ± 49.8, *z* = −6.2, *p* = 1.000). The stratified Markov null generates even more diffuse topology than the global null, further strengthening the Markov-1 non-rejection. We note that the GMM regime structure at the point-cloud level used for this test collapsed to essentially two active regimes (one containing 99.9% of trajectories), limiting the effective stratification; per-regime Markov testing with more balanced partitions is a direction for future work.
...
An explicit caveat is warranted here: topological Markov-1 consistency establishes that the trajectory geometry is first-order under current conditions, but this does *not* imply that policy interventions targeting transition probabilities will produce predicted regime shifts. The topology is consistent with Markov-1 under the *current* transition matrix; interventions would change the transition matrix itself, and the topological consequences of such changes are not tested here. The relationship between a modified transition matrix and the resulting topology of the trajectory point cloud is non-trivial and would require simulation-based counterfactual analysis to establish.
...
The H₁ results require direct discussion. As reframed in §2.3, H₁ loops correspond to cyclical oscillation patterns in the embedding space — trajectories that trace closed paths through state configurations — rather than to poverty traps in the absorbing-state sense. Our results are nuanced: at $L = 5{,}000$, order-shuffle H₁ is significant (*p* = 0.020), indicating temporal ordering creates detectable loop structure at this landmark resolution. However, this result is landmark-sensitive — it does not replicate at $L = 2{,}500$ (*p* = 0.20) or $L = 8{,}000$ (*p* = 0.14) — and is embedding-sensitive, appearing under only two of six tested embedding configurations (§5.4). Markov-1 H₁ (*p* = 1.000) and Markov-2 H₁ (*p* = 0.735) confirm that whatever loop structure exists is fully consistent with Markov transition dynamics.
...
This negative result bounds the complexity of cyclical dynamics: the trajectory manifold lacks persistent cyclical structure beyond what first-order Markov dynamics produce. Claims about non-Markovian topological trapping cycles would be unsupported by these data. It also directs analytical attention toward the stronger H₀ results, where the topological signal is firmly established, and toward the escape-rate analysis (§4.4.2), which addresses the absorbing-state dimension of disadvantage through regime persistence rather than through H₁ loop structure.
...
To systematically assess embedding sensitivity, we test a grid of six configurations varying n-gram order (bigram, trigram), weighting (TF, TF-IDF), and PCA dimensionality (10, 20, 30), each with order-shuffle null testing at $L = 3{,}000$ landmarks:
...
H₀ is uniformly significant across all six configurations (*p* < 0.001), confirming that the connected-component structure — our primary positive finding — is robust to embedding choices. H₁ is embedding-sensitive: significant for trigram TF-IDF (*z* = +4.0, *p* < 0.001) and borderline for the baseline bigram TF (*p* = 0.040), but clearly non-significant for bigram TF at PCA-10 (*p* = 0.920) and PCA-30 (*p* = 0.840). The trigram TF-IDF result is theoretically noteworthy: trigrams directly encode three-state transition memory, and the H₁ significance under this embedding suggests that higher-order sequential structure creates detectable loop topology — a connection to the Markov-2 null that warrants further investigation. The H₁ sensitivity to embedding configuration reinforces our cautious interpretation of the H₁ order-shuffle result in Table 2.
...
**Cross-survey harmonisation.** BHPS and USoc use different sampling frames, questionnaire designs, and income measurement methodologies. BHPS reports raw household income (`fihhmn`) requiring manual equivalisation via the modified OECD scale, while USoc provides pre-equivalised income (`fihhmnnet1_dv`). Harmonisation artefacts may contribute to cross-era topological differences. The primary analysis is drawn from the USoc period (start years 2009–2013); BHPS-era trajectories (1991–2008) serve as cross-era validation but cover a different labour market regime (pre-crisis full employment versus post-crisis austerity). Even with genuine BHPS-era trajectories, cohort effects remain partially confounded with age in a single-country panel: different birth cohorts are observed at different life stages in each era. The BHPS-era replication (§4.7) mitigates but does not fully resolve this age-cohort identification problem.
...
| Embedding sensitivity grid | Completed (§5.4) | 6 configs: {bigram,trigram} × {TF,TF-IDF} × {PCA-10/20/30}; H₀ *p* < 0.001 uniformly; H₁ embedding-sensitive |
| Non-overlapping windows | Completed (§4.4.2) | Self-transition: overlap 0.686 vs non-overlap 0.601; escape rate 5.6% vs 11.0%; core structure preserved |
...
| Bootstrap null stability | Completed (§4.3) | 10 draws × 50 perms: order-shuffle H₀ 10/10 significant; Markov-1 H₀ 10/10 significant; no conclusion flips |
| Intra-regime compactness | Completed (§4.3) | All 7 regimes significantly more compact in observed vs order-shuffled (*z* = −30 to −68, all *p* < 0.001) |
| Stratified Markov-1 | Completed (§5.2) | Per-regime TMs (*n* = 20, *L* = 5,000): observed persistence lower than stratified null; Markov-1 non-rejection strengthened |
| NS-SEC missingness | Completed (§3.1) | 39.1% missing; χ² = 147.74, *p* < 10$^{-29}$; non-random; stratified results require caution |
| BHPS cross-era replication | Completed (§4.7) | *n* = 8,509; order-shuffle H₀ *p* = 0.000; Markov-1 *p* = 1.000; same qualitative pattern as USoc |
| Age-adjusted escape rates | Completed (§4.4.2) | Working-age escape 17.8%; retirement 0.1%; extended logistic pseudo-R² = 0.479 |
...
This paper presents the first application of persistent homology to longitudinal life-course trajectories, analysing 27,280 employment-income sequences from Understanding Society (2009–2023) in a 20-dimensional PCA embedding space, with cross-era validation on 8,509 BHPS-era trajectories (1991–2008).

**Positive result.** Seven distinct mobility regimes emerge with statistically strong, order-dependent clustering: the order-shuffle null is rejected at *p* < 0.005 for H₀ total persistence, confirming that temporal ordering of states within trajectories creates genuine topological structure that exceeds what state composition alone would produce. Intra-regime compactness testing confirms that this temporal structure contributes to regime differentiation (all seven regimes *p* < 0.001 under order-shuffle). The seven regimes range from Secure High-Employment (27%, stability 0.78) to Low-Income Churn (7.6%, stability 0.29), with a strong correlation between regime stability and income ($r$ = 0.77). Comparison with OM (dynamic Hamming) clustering at matched *k* = 7 yields ARI = 0.26, confirming that the broad regime typology is robust to method while TDA adds formal null testing and multi-scale geometric characterisation.

**Complexity bound.** The Markov memory ladder shows that regime topology is not rejected by first- or second-order Markov dynamics (Markov-1 H₀ *p* = 1.000, Markov-2 H₀ *p* = 1.000). Markov surrogates generate *more* total persistence than observed, indicating that real trajectories are more structured and constrained than Markov processes would produce. The topology is real but consistent with one-step transitions. Phase-ordering analysis with *n* = 500 permutations reinforces this finding: the temporal ordering of career phases adds no topological structure beyond the within-trajectory sequential signal (H₀ *p* = 0.98, 95% CI [0.968, 0.992]). A per-regime stratified Markov-1 null further strengthens the non-rejection (*z* = −15.9, *p* = 1.000). Bootstrap null stability analysis (10 draws) confirms that no H₀ conclusion flips across regime re-assignments. Embedding sensitivity testing across six configurations confirms H₀ significance is robust (all *p* < 0.001).

**Negative result.** H₁ cycles — cyclical oscillation patterns in the embedding space (§2.3) — show landmark-sensitive significance under order-shuffle (*p* = 0.020 at $L = 5{,}000$, non-significant at other $L$ values) and are consistent with Markov transition structure (*p* = 1.000 for both Markov-1 and Markov-2). This bounds the complexity of cyclical dynamics: any persistent loop structure is explicable by first-order transition probabilities. H₁ is embedding-sensitive, showing significance under trigram TF-IDF but not under most other configurations.

**Cross-era replication.** The BHPS-era analysis (8,509 trajectories, 1991–2008) reproduces the qualitative pattern: order-shuffle H₀ *p* = 0.000, Markov-1 *p* = 1.000. Spanning trajectories (8,459 individuals observed in both surveys) confirm the same finding. This cross-era replication across two different survey instruments and two different labour market eras substantially strengthens the conclusion that trajectory topology is bounded by first-order Markov dynamics.

**Stratification.** Wasserstein distances reveal origin-dependent topology: gender differences are significant for H₀ cluster structure; parental NS-SEC differences are significant for all three class pairings in H₀; and birth cohort produces the most extensive differentiation, with 26 of 42 cohort tests significant. All 30 significant results (of 50 total tests) survive Benjamini-Hochberg FDR correction at *q* < 0.05 (H₀: 16/25, H₁: 14/25). NS-SEC missingness is non-random (39.1% missing, $\chi^2$ = 147.74, *p* < 10$^{-29}$), and NS-SEC-stratified results should be interpreted with this caveat.
...
**Regime stickiness.** Overlapping career-phase windows confirm extreme regime persistence: the 7 × 7 transition matrix is diagonal-dominant, and only 5.6% of individuals starting in disadvantaged regimes escape to advantaged regimes within a single window transition (11.0% under non-overlapping windows, confirming that window overlap inflates persistence by ~8 percentage points but does not create the finding). An extended logistic regression (pseudo-$R^2$ = 0.479) reveals that initial regime (Regime 6 OR = 20.6) and birth cohort (1960s OR = 23.8) are the dominant predictors of escape, while parental NS-SEC is not significant after controlling for structural entry position.
...
- **Order-shuffle H₁**: Significant at *L* = 5,000 (*p* = 0.020) but non-significant at *L* = 2,500 (*p* = 0.20) and *L* = 8,000 (*p* = 0.14). The H₁ result is therefore landmark-sensitive: at the higher resolution of L=5,000, temporal ordering creates detectable loop structure, but this signal does not replicate consistently across landmark counts. We interpret the H₁ order-shuffle result cautiously.
