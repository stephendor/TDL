...
**U.S.-Centric Analysis**: All three validated crises are U.S. market events, though with global spillovers. International markets may exhibit different correlation dynamics due to distinct financial structures (bank-centric vs market-centric), regulatory frameworks, and economic cycles. European markets experienced sovereign debt crises (Greece 2010-2015) not mirrored in the U.S.; Chinese equity market volatility (2015 Shanghai crash) differs from Western patterns; emerging markets face unique contagion vulnerabilities. Validation on STOXX 600 (Europe), Nikkei 225 (Japan), Hang Seng (Hong Kong), and MSCI Emerging Markets would assess geographic transferability.
...
**Look-Ahead Bias in Parameter Optimization**: Our parameter optimization used the known crisis dates to search over (W, P) space. In real-time prospective analysis, the crisis date is unknown, creating a challenge: we cannot optimize parameters to maximize pre-crisis τ without knowing when the crisis will occur. The ensemble approach partially addresses this (test multiple parameter sets simultaneously), but more sophisticated online methods could adapt parameters dynamically based on trailing performance. Bayesian optimization or reinforcement learning frameworks might update (W, P) monthly based on recent signal quality, though this introduces risk of overfitting to noise.
