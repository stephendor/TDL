"""
Phase 10B - Task B.2: HMM Baseline (VIX only)
=============================================

Purpose:
    Fit a 3-state Gaussian HMM using returns and VIX only (no τ).
    This serves as the baseline for comparing the τ-augmented model.
...
Output:
    ...
    outputs/phase_10b/hmm_baseline_report.md
...
STATES_FILE = OUTPUT_DIR / "hmm_baseline_states.csv"
REPORT_FILE = OUTPUT_DIR / "hmm_baseline_report.md"
...
def prepare_baseline_features(df):
    """Prepare features for baseline model (no τ)."""
    # Baseline: returns + VIX only
    baseline_cols = ["ret_1d", "ret_5d", "ret_21d", "vix", "vix_5d_chg"]
    X = df[baseline_cols].values
    ...
    print(f"  Baseline features: {baseline_cols}")
...
def fit_hmm(X, n_states=N_STATES):
    """Fit Gaussian HMM."""
    print(f"\nFitting {n_states}-state Gaussian HMM...")
    ...
    # Model metrics
    n_params = n_states * (n_states - 1) + n_states * X.shape[1] + n_states * X.shape[1] * (X.shape[1] + 1) // 2
...
def label_states(model, states, df):
    ...
    for s in range(N_STATES):
        ...
        state_vix.append((s, mean_vix, mean_ret))
    ...
    for s, vix, ret in state_vix:
        print(f"  State {s} ({state_labels[s]}): VIX={vix:.1f}, Mean Ret={ret*100:.3f}%")
...
def generate_report(df, states, metrics, state_labels, crisis_results):
    ...
    report = []
    report.append("# HMM Baseline Model Report")
    report.append("")
    ...
    report.append("| Features | ret_1d, ret_5d, ret_21d, vix, vix_5d_chg |")
    report.append("| τ included | **No** (baseline) |")
    ...
    report.append("---")
    report.append("*Baseline model for comparison with τ-augmented model*")
...
def main():
    """Main execution."""
    print("=" * 60)
    print("PHASE 10B - TASK B.2: HMM BASELINE (VIX ONLY)")
    print("=" * 60)
    ...
    X, X_mean, X_std = prepare_baseline_features(df)
    ...
    # Save states
    output_df = df[["state", "state_label"]].copy()
