"""
2008 Global Financial Crisis detection validation script.

This script validates the TDA crisis detection system against the 2008 GFC,
measuring lead time, precision, recall, and F1 score for regime change detection.
Generates visualizations and metrics for checkpoint review.

Validation period: 2007-01-01 to 2009-12-31
Key event: Lehman Brothers collapse (2008-09-15)
"""
...
def fetch_gfc_data() -> tuple[dict[str, pd.Series], pd.Series]:
    """
    Fetch multi-index data for 2008 GFC validation.
    ...
    """
    logger.info("Fetching 2008 GFC validation data...")

    # Fetch 4 major indices (Gidea & Katz approach)
    # Use short names for fetch_index function
    index_names = ["sp500", "dji", "nasdaq"]

    # Fetch data from 2007-01-01 to 2009-12-31
...
def calibrate_normal_period(distances: pd.Series, crisis_start: pd.Timestamp) -> NormalPeriodCalibrator:
    ...
    Args:
        ...
        crisis_start: Start date of crisis (Lehman collapse: 2008-09-15).
    ...
    # Use pre-crisis period (before Lehman collapse) as "normal"
...
def plot_bottleneck_timeline(
    ...
    ax.axvline(crisis_start, color="black", linestyle=":", label="Lehman Collapse")
    ...
    ax.set_title("2008 GFC: Bottleneck Distance Timeline", fontsize=14, fontweight="bold")
...
def generate_validation_report(
    ...
    report = f"""# 2008 Global Financial Crisis Validation Report

## Executive Summary

This report validates the TDA-based crisis detection system against the 2008 Global Financial Crisis (GFC), focusing on the Lehman Brothers collapse event (September 15, 2008).

**Key Findings:**
- **Lead Time**: {metrics["lead_time_days"]} days before Lehman collapse
...
- **Onset**: {crisis_info["onset_date"].strftime("%Y-%m-%d")} (Lehman collapse)
- **Peak Drawdown**: {crisis_info["peak_drawdown_date"].strftime("%Y-%m-%d")} (S&P 500 bottom)
- **Recovery**: {crisis_info["recovery_date"].strftime("%Y-%m-%d")}
...
- `gfc_2008_bottleneck_timeline.png`: Time series of bottleneck distances with crisis period highlighted
- `gfc_2008_persistence_comparison.png`: Persistence diagrams comparing pre-crisis vs crisis topology

## Interpretation

### Strengths
- Multi-index approach captures market-wide structural changes
- Bottleneck distance effectively quantifies topological divergence
- Pre-crisis calibration provides statistically grounded thresholds

### Limitations
- Sliding window approach has inherent lag (window_size/2 days)
- Threshold tuning (percentile selection) affects precision-recall tradeoff
- Does not distinguish crisis types (credit, liquidity, volatility)

### Observations
The 2008 GFC validation provides insights into the detection system's ability to identify gradual, systemic crises. The lead time metric indicates whether the system can provide actionable early warning before peak market stress.

## Conclusion

{"✓ PASS" if (metrics["lead_time_days"] and metrics["lead_time_days"] >= 5 and metrics["f1_score"] >= 0.70) else "✗ FAIL"}: The TDA crisis detection system {"meets" if (metrics["lead_time_days"] and metrics["lead_time_days"] >= 5 and metrics["f1_score"] >= 0.70) else "does not meet"} success criteria for the 2008 GFC event.
...
def main():
    """Run 2008 GFC validation."""
    logger.info("=" * 80)
    logger.info("2008 GLOBAL FINANCIAL CRISIS VALIDATION")
    logger.info("=" * 80)

    # Fetch data
    prices, vix = fetch_gfc_data()
    ...
    # Get crisis metadata
    gfc_crisis = [c for c in KNOWN_CRISES if c.name == "GFC"][0]
    crisis_info = {
        "onset_date": gfc_crisis.onset_date,
        "peak_drawdown_date": gfc_crisis.peak_drawdown_date,
        "recovery_date": gfc_crisis.recovery_date,
    }

    # Compute bottleneck distances
    distances = compute_bottleneck_distances(prices, window_size=50, stride=1)

    # Calibrate normal period (pre-Lehman)
    calibrator = calibrate_normal_period(distances, gfc_crisis.onset_date)
    ...
    metrics = compute_detection_metrics(
        detections,
        (gfc_crisis.onset_date, gfc_crisis.recovery_date),
    ...
    plot_bottleneck_timeline(
        ...
        (gfc_crisis.onset_date, gfc_crisis.recovery_date),
        threshold,
        FIGURES_DIR / "gfc_2008_bottleneck_timeline.png",
    )

    plot_persistence_comparison(
        prices,
        pre_crisis_date=pd.Timestamp("2008-08-01"),  # 6 weeks before Lehman
        crisis_date=pd.Timestamp("2008-10-15"),  # 1 month after Lehman
        window_size=50,
        output_path=FIGURES_DIR / "gfc_2008_persistence_comparison.png",
    )
    ...
    generate_validation_report(
        ...
        detections,
        crisis_info,
        Path(__file__).parent / "2008_gfc_validation.md",
    )

    logger.info("=" * 80)
    logger.info("2008 GFC VALIDATION COMPLETE")
    logger.info("=" * 80)
