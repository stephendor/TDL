...
def test_two_circles_have_two_components(sample_point_cloud):
    ...
    assert 1 <= n_components <= 3, \
        f"Expected ~2 components, got {n_components}"
...
def test_persistence_reproducibility(sample_point_cloud):
    ...
    np.testing.assert_array_equal(
        diagrams1, diagrams2,
        err_msg="Persistence diagrams not reproducible"
    )
...
def test_persistence_with_noise_perturbation(sample_point_cloud):
    ...
    # (stability theorem applies to all features, but most persistent are easiest to validate)
    if len(clean_h1) > 0 and len(noisy_h1) > 0:
        ...
        assert relative_change < 0.5, \
            f"Persistence changed by {relative_change:.1%} under small noise (expected < 50%)"
