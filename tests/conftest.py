...
def assert_persistence_diagram_valid(diagram: np.ndarray) -> None:
    ...
    assert diagram.shape[1] == 2, f"Diagram must have 2 columns [birth, death], got {diagram.shape[1]}"
...
def assert_betti_numbers_match(
    computed: np.ndarray,
    expected: np.ndarray,
    tolerance: float = BETTI_TOLERANCE
) -> None:
    """
    ...
    if np.any(nonzero_mask):
        ...
        assert max_error <= tolerance, \
            f"Betti numbers exceed tolerance: max relative error = {max_error:.3f} > {tolerance}"
...
def assert_bottleneck_distance_within(
    ...
    try:
        ...
        distance = wasserstein_distance(
            diagram1, diagram2, order=np.inf, internal_p=2
        )
        ...
        assert approx_distance <= threshold, \
            f"Approximate bottleneck distance {approx_distance:.6f} exceeds threshold {threshold}"
